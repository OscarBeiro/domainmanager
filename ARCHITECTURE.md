...
|---|---|---|---|
| `Hooks::POST_ITEM_FORM` | `Domain` | `DomainForm::inject()` | Renders `domain_panel.html.twig` inside the form: **Registrar**, read-only (`fields.readOnlyField()`, native form styling, §0.1) with a link to the Infocom tab, **DNS Provider** read-only display (detected name, linked to supplier when resolved), **status card** (last sync date + green/red Registrar/DNS badges), **Update Now** button, unsupported/unknown warning + "Help us support this provider" link, and the lock-disabling JS. Rendered only with `domain` READ. |
