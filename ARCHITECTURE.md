...
7. **Phase 7a (implemented 2026-07-21)** — richer registrar domain metadata in the inventory. Implementing the real `IonosDriver` registrar pipeline (§3.9) surfaced that its Domains API response (`domainLarge`) carried several fields this plugin fetched but discarded, since `DomainLifecycle` only modeled `registrationDate`/`expirationDate`/`status`: **`authInfo`** (EPP transfer/auth code), **`privacyEnabled`**, **`domainLock`**, **`transferLock`**, **`autoRenew`**, **`domainType`**, **`dnsSecEnabled`**.
   ...
   **Per-driver support, confirmed live against each driver's real API (never guessed) before implementing anything, per this section's own original mandate:**
   ...
   - **Also planned for Phase 7 (addendum "Searchable 'Proxy Status' Field for CDN-Proxied Records"), not started:** extend the `glpi_plugin_domainmanager_records` table (§5.7 — reusing the same table `is_managed` lives on, not a separate one, for the same "don't add a redundant abstraction" reason) with a second field, `is_proxied`, capturing whether a DNS record is CDN-proxied (Cloudflare's "orange cloud" vs "grey cloud") — a genuine **three-state** value, not a plain bool: `1` proxied, `0` DNS-only-but-proxy-eligible, `NULL` not applicable at all (any non-Cloudflare record, or a Cloudflare record type Cloudflare itself never allows proxying for).
     - **Eligible types confirmed live** (not assumed): only **A, AAAA, and CNAME** can be proxied on Cloudflare — MX, TXT, SRV, CAA, and NS have no proxy toggle at all (community/vendor documentation cross-checked, e.g. stackharbor.com's per-record-type breakdown). `CloudflareDriver::fetchZoneRecords()` already receives each record's `proxied` attribute from Cloudflare's API (confirmed in the live OpenAPI spec: `proxied` is a real field on every DNS record type's schema, `github.com/cloudflare/api-schemas`) — populate `is_proxied` from it for A/AAAA/CNAME, leave it unset/`NULL` for every other type and for every non-Cloudflare driver (IONOS/Dinahosting never populate `is_proxied` — never write `0` for "not applicable").
     - **Tri-state search rendering — promising lead found, not yet fully verified end-to-end:** confirmed directly in `Glpi\Search\SearchOption::getSearchOptionsTypes()`/equivalent on `11.0/bugfixes` that the `bool` datatype's search-type list already includes a distinct **`'empty' => __('is empty')`** option alongside `equals`/`notequals` — meaning a plain nullable `bool`-typed search option may already give genuine three-state filtering (Yes / No / Is Empty) natively, without needing a custom `dropdown`-style datatype with three defined values. This needs a real end-to-end check (create a NULL-valued row, confirm the "is empty" filter actually isolates it, and confirm the results-table display renders `NULL` as a visually distinct blank rather than coercing it to "No") before relying on it — the same "confirm before implementing" standard applied to `is_managed`'s join shape.
     - Label **"Proxy status"** (no plugin-name prefix, same convention as "Managed", §5.7).
8. **Phase 8 (planned, not started)** — bulk-import undiscovered registrar domains. Scenario: a configured registrar Supplier's real account has domains that don't exist as native `Domain` assets yet, with no way today to see the gap or bulk-create them.
   - **New capability interface**, `Contract/DomainDiscoveryInterface` (`listAccountDomains(): DiscoveredDomain[]`), a `DiscoveredDomain` DTO (name + cheap preview metadata already returned by the registrar's own list call — no per-domain detail fetch just to build the list), and pagination handling verified per-driver rather than assumed. **Any driver may implement it, including `CloudflareDriver`** — Cloudflare Registrar is at-cost-only and only for domains *already* on Cloudflare DNS (a DNS-hosted zone is not necessarily registrar-managed there), so its implementation must call the **Registrar API specifically** (`accounts/{account_id}/registrar/domains`-shaped, to be confirmed live like every other Cloudflare endpoint in this plugin) and never fall back to the full zone list, which would wrongly present ordinary DNS-only zones as importable registrar domains. Errors route through the same `ConnectionTestResult`-style classification and the same two log files as everywhere else in this plugin.
   - **UI**: an "Import Domains" button on the Supplier tab, shown purely via an `instanceof DomainDiscoveryInterface` capability check (never a per-driver allowlist) — so it appears for Cloudflare automatically once/if that driver implements the interface, with no separate UI wiring needed. Opens a modal (same ribbon-banner + native-table convention as every other panel, §6.5) listing discovered domains against a **global** (cross-entity) case-insensitive/trimmed existence check against `glpi_domains` — not present → checkable (checked by default); present → grayed out with a link, annotated "Registrar mismatch" (not a bare "already exists") when the existing Domain's stored `suppliers_id` disagrees with the API's answer for who actually holds it, with an inline one-click "Reassign registrar to X" action (updates `suppliers_id` + `Log::history()`, same rights check as the credentials tab). An entity selector applies to the whole import batch. Creation sets `name`/`entities_id`/the seeded "Internet Domain" type/`suppliers_id`, deliberately pre-locks nothing (locking only ever applies after a domain's first real sync, an existing rule — see §0.3/§5), and should enqueue an initial `SyncEngine::sync()` per newly created domain if that's a small addition on top of the existing single-domain sync path; otherwise leave them "Not yet checked" as a documented followup rather than blocking the phase on it.
   - **Rights**: reuses the existing config-level right already gating the credentials tab (no new right); additionally requires native `Domain::canCreate()` — if a user can configure suppliers but can't create Domains, the import action must fail with a clear message, not silently.
   - **"Registrar mismatch" as a first-class, generalized state** (not just an import-modal detail) — this formalizes the same underlying situation already hit once for real (§3.8's Dinahosting `pontecm.com`/code-2201 case: valid credentials, wrong domain-to-account attribution). Principle: **the registrar API's answer about who holds a domain always wins over GLPI's stored `suppliers_id`** — when they disagree, say so plainly and offer the one-click reassign fix, not just a discrepancy note. Applies in two places: the import modal (above), and the Domain form's existing Registrar sync status panel (§6.2) — offering the same one-click reassign there too, but *only* when the correct registrar can actually be determined from the API's response (e.g. Phase 8's own discovery data already knows who holds it); when a driver's error only says "not authorized" with no hint of the real owner (the original Dinahosting case), keep the existing detailed-error treatment from that addendum rather than fabricating a fix target that isn't supported by any real data. Needs its **own distinct visual treatment** — not plain red "Error" (this is fixable in one click, not a dead end) and not as calm as "Supplier inactive" (this reflects genuinely incorrect stored data, not an intentional, expected state) — exact color/icon choice to be decided and documented here when this phase is actually implemented.
   - **NS provider registry additions** (`resources/ns-providers.json`, §4) for detection-only coverage (no driver): **Openprovider, Hostinger, OVHcloud** (check for a pre-existing entry from the earlier popular-provider sweep, §5, before adding a duplicate), **Aruba, OpusDNS** — real, currently-verified nameserver wildcard patterns sourced from each provider's own documentation (never guessed), with sources documented in the registry file's comments per the existing convention, so these render as "detected, not currently supported" (name + the existing GitHub contribution link) instead of "Unknown."
