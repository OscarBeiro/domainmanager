...
See `TESTING-dev.md` §15 for both regression checks, and the versioning-requirement note in the `glpi-plugin-builder` skill this project uses — every version bump must land in the same commit as the migration/feature it gates, not a separate later commit.
...
|---|---|---|---|
| `Hooks::POST_ITEM_FORM` | `Domain` | `DomainForm::inject()` | Renders `domain_panel.html.twig` inside the form: a ribbon-banner header (§6.5) whose title is the domain name itself, made a clickable outbound link with a trailing `ti-external-link` icon (§6.5's outbound-link convention) rather than a separate "Visit" button. Below it, a topic-grouped field/value grid (§6.5, Phase 94) separated by `hr-text` dividers — "Providers & key dates" (Registrar, DNS/NS Provider, Registrar sync, DNS sync, Last changed, Last transfer, Reg/DNS last sync, RDAP last sync), replacing the earlier one-row `<table>` layout. Registrar/DNS Provider are hyperlinked to the resolved Supplier's own Domain Manager tab when one exists (else plain text/muted fallback, with a link to the Infocom tab for Registrar, §0.1). Each of the Registrar sync/DNS sync badges carries its own detail message as a real GLPI-styled tooltip (`data-bs-toggle="tooltip"`) rather than a separate always-visible text line below the grid — the line was removed in a Phase 94 follow-up once it turned out to just duplicate what the badge's own `title` already carried. Below the grid: the unsupported/unknown provider warning, plus the lock-disabling JS. When `registrar_status == 'ok'`, a further "Registrar security flags" group (§9 Phase 7) renders the same grid convention for WHOIS privacy / transfer lock / domain lock / auto-renew / DNSSEC / pending delete / pending transfer (shared badge component, or "Not reported by this driver" when `NULL`) — no RDAP-vs-driver distinction in the grouping (Phase 94). WHOIS and (when applicable) Unlink registrar are relocated (not rendered in their own row) into the Domain form's own native Save/Put in trashbin button row, same DOM-relocation technique as the historical "Update Now" button below (§6.5's button-row convention, Phase 94). The transfer/EPP auth code badge that used to sit in this section was removed in Phase 91 — the feature (fetch + persist + masked display) was dropped entirely, not just hidden. Rendered only with `domain` READ. |
...
- **Single-record read-only summary**: core's own field/value grid, from `inventory_info.html.twig`'s body — `<div class="card-body row"><div class="mb-3 col-12 col-sm-4"><label class="form-label">Label</label><span>Value</span></div>...</div>`. The Domain form's "Domain Manager" panel uses this grid (`col-sm-3`, 4 per row — the panel is field-dense enough that 3-per-row read as too wide), **topic-grouped** into labeled sub-sections ("Providers & key dates", "Registrar security flags") rather than one flat grid or the earlier one-row table it used before Phase 94 — grouping is by what a viewer would look up together, not by field provenance (driver-reported vs. RDAP-enriched fields are deliberately mixed in the same group once their values converge, since the source doesn't matter to the reader).
...
When a read-only value's whole purpose is to link elsewhere (an external URL, a link to another Supplier's tab), the *value itself* is the `<a>` — confirmed as core's own idiom in `templates/pages/setup/marketplace/card.html.twig` and `templates/components/itilobject/timeline/document/document_list_item.html.twig`. The Domain form panel's title (the domain name) is itself `<a href="https://{{ domain }}" target="_blank" rel="noopener noreferrer">` (Phase 94) rather than a separate small "Visit" button next to plain text — no redundant click affordance next to the link. A plain-colored link with no visual affordance reads as static text, not a link, so a small trailing `<i class="ti ti-external-link">` icon stays alongside it — the icon signals "this leaves GLPI", the coloring/underline signals "this is clickable"; dropping either one made the very first live-rendered version of this panel confusing (caught by the user, not by grepping core — core's own outbound-link examples above are single-purpose icon links or plain inline text links, not a panel *title* doing double duty as a link, so there was no exact precedent to copy verbatim here).

### Section dividers inside one card body: `hr-text`, not a bare `<hr>`
When a panel's body has more than one topic-grouped section (§6.5's grouped-field body shape) and needs a visible divider between them, use Tabler's `hr-text` component — confirmed live in core's own `templates/pages/setup/general/general_setup.html.twig` (and `dbreplica_setup.html.twig`, `preferences_setup.html.twig`): `<div class="hr-text"><i class="ti ti-<icon>"></i><span>{{ __('Section name') }}</span></div>` renders a horizontal rule with the icon+label centered on it. This plugin's domain panel used to separate sections with a bare `<hr class="m-0">` plus a separate `text-muted small text-uppercase` label above the next section — **that `<hr>` pattern does not exist anywhere in GLPI 11 core** (confirmed by grep, zero hits), it was this plugin's own invention predating this doc's UI-conventions work, not a real core convention. Fixed in Phase 94 to use `hr-text` instead, folding the group heading into the divider itself rather than a separate line above it.

### Panel action buttons: reuse the item's own native button row, don't invent a new one
A panel with more than one action (WHOIS lookup, Unlink registrar, etc.) does **not** get its own bottom action row inside the panel's `card-body` — it reuses the exact same DOM-relocation technique already established for the earlier "Update Now" button (§9 Phase 9 addendum, below): render each button `d-none` inside the panel, then a small script moves the actual DOM node into the item's own native `.form-button-separator` row (`components/form/buttons.html.twig`) alongside Save/Put in trashbin, styled `btn-outline-secondary` to match. A plugin-invented button row next to (but visually separate from) the item's real Save/Trashbin row reads as a second, competing action bar — the first live-rendered version of Phase 94 did exactly this and the user correctly flagged it. One correction versus the original Update Now implementation's own doc (below): that write-up says the relocation script is "deferred to `DOMContentLoaded`" — **confirmed live in Phase 94 that this doesn't fire reliably**, because the Domain form's tabs (including the main tab holding this panel) are lazy-loaded via AJAX (`data-glpi-ajax-content` / `ajax/common.tabs.php`) *after* the outer page's own `DOMContentLoaded` has already fired once; a listener registered for it inside AJAX-injected markup never runs. The working fix is a plain immediately-invoked function with no event listener at all — the native `.form-button-separator` row is already present in the DOM by the time this panel's own `<script>` tag is evaluated (it's part of the same already-rendered tab-content payload), so no deferral is needed or safe to rely on.
...
**TESTING-dev.md line**, not a reason to build restore-time interception.
...
| **35** | `1.2.0-beta1` | End-to-end verification; finalise `ARCHITECTURE.md`, `CHANGELOG.md`, `TESTING-dev.md`. Refining only, nothing new built (implemented 2026-07-29) |
...
- **Live addendum (2026-07-29, same day):** `glpi-claude` (port 65008) brought up and the
  ...
  **not** performed — deliberately deferred, not silently skipped, same as the live-driver-call
  ...
  read-only pass. See `TESTING-dev.md` §35.17 for the full detail. §11.15's table marks Phase 35 as
...
on this branch (commits `11f381c` "Verify Phase 49 per-type purge right live, check off TESTING-dev.md"
...
If the cause is provider behaviour, the outcome is a documented limitation in §4 plus a `TESTING-dev.md`
...
consulted. If both are already correct, the phase concludes with `TESTING-dev.md` regression checks and no
code change — but per the `hasPurgeRight()` precedent, "looks right" is not sufficient here.

**Outcome, 2026-08-03 (TESTING-dev.md Phase 52):** `writableTypes()`/`writableSupplierName()` had
...
**Outcome, 2026-08-03 (TESTING-dev.md Phase 53):** Implemented as `Config::isReadOnlyMode()`/
...
**Outcome, 2026-08-03 (TESTING-dev.md Phase 55):** `RecordReconciler::doReconcile()` counts, after its
...
5. `TESTING-dev.md`: a rotation regression case running enough consecutive ticks to exceed one full
...
5. Add test cases to `TESTING-dev.md`: verify users without the right cannot toggle via the form (both that the control is absent/disabled in rendered HTML, and that a raw POST with `_domainmanager_proxied` is still rejected server-side); verify error message is logged.
...
Not done as part of this phase: `TESTING-dev.md` test-case entries (§17.11 point 5) and live end-to-end
...
TESTING-dev.md's Phase 88 entry for the live verification matrix.
...
entry; regression checklist in `TESTING-dev.md`'s "Phase 91" section.
