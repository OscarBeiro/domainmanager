    ...
    public function __invoke(int $suppliers_id, Request $request): Response
    {
        ...
        foreach ($names as $normalized => $name) {
            ...
            if (isset($trashed[$normalized])) {
                ...
                //
                // HookHandler::domainSaved() already runs synchronously off
                // `$domain->add()` above and creates the state row itself
                // when the new domain has a Punycode/TLD value to cache —
                // which is effectively always. Adding a second row for the
                // same domains_id would violate the unique key, so update
                // the existing row instead of assuming none exists yet.
